...
def split_and_filter_vcf(
    ...
    Args:
        cfg (DictConfig): Application configuraiton
        inference_vcf (str): Input mulit-sample VCF
    ...
    # Lazily create per-sample writers across all regions
    writers = {}
    ...
    for _region_count, (region, variants) in enumerate(overlapping_variants(reader, flank=cfg.pileup.variant_padding), start=1):
        ...
        negative_samples = possible_samples - samples_with_any_sv
        ...
        for sample in positive_samples | negative_samples:
            if sample not in writers:
                sample_path = os.path.join(output_dir, f"{sample}.vcf.gz")
                writers[sample] = VariantFileWriter.open(sample_path, header)
            for variant in passing_variants:
                writers[sample].write(variant)
