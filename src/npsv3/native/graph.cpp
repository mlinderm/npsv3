...
void Haplotype::AddReferenceNodes(size_t end_index) {
  if (next_ref_index_ >= end_index) return;  // Nothing to add
...
HaplotypeAddSteps::HaplotypeAddSteps(const Haplotype& haplotype) : curr_next_ref_index_(haplotype.next_ref_index_) {
  // Check if path is emtpy to avoid recording invalid step handle
  if (haplotype.graph_.is_empty(haplotype.current_segment_handle_)) {
    curr_step_ = haplotype.graph_.path_front_end(haplotype.current_segment_handle_);
  } else {
...
HaplotypeSamplerOverlay::HaplotypeSamplerOverlay(const Graph& graph, size_t k, size_t max_edge)
    : HaplotypeSamplerOverlay(graph, k, max_edge, Params{}) {}

HaplotypeSamplerOverlay::HaplotypeSamplerOverlay(const Graph& graph, size_t k, size_t max_edge, Params params)
    : graph_(graph), params_(params) {

  graph.UniqueKmers(
      k, max_edge,
      [&](const std::string& seq, const std::vector<handlegraph::handle_t>& handles, uint64_t offset) {
        auto zyg = KmerZygosity::HOMOZYGOUS; // Placeholder for actual zygosity determination based on the paths associated with the k-mer
        double score = params_.homozygous_score; // Placeholder for actual zygosity-based scoring

        // Need to wait for C++20 for parenthesized initialization in emplace_back
        auto kmer_idx = kmers_.size();
        kmers_.push_back({seq, zyg, score});
        ...
        if (handles.size() == 1) {
          ...
          auto& edges = shadow_edges_[graph.get_id(handles.front())];
          auto edge_it = std::find_if(edges.begin(), edges.end(), [&](const ShadowEdge& se) {
            // A matching node must have the same nodes (the first is already checked by the map key).
            if (se.nodes.size() != handles.size() - 1) return false;
            for (size_t i = 0; i < se.nodes.size(); i++) {
              if (se.nodes[i] != graph.get_id(handles[i + 1])) return false;
            }
            return true;
          });

          if (edge_it == edges.end()) {
            // No existing shadow edge matches this handle path, create a new one
            ShadowEdge se;
            se.nodes.reserve(handles.size() - 1);
            for (size_t i = 1; i < handles.size(); i++) {
              se.nodes.push_back(graph.get_id(handles[i]));
            }
            se.kmer_indices.push_back(kmer_idx);
            edges.push_back(std::move(se));
          } else {
            // Existing shadow edge matches this handle path, add k-mer index to it
            edge_it->kmer_indices.push_back(kmer_idx);
          }
        }
      },
      true /* exclude 'universal' k-mers in each haplotype */);

  // Fill in k-mer indices for intermediate nodes in shadow edges
  for (auto& [_, edges] : shadow_edges_) {
    for (auto& edge : edges) {
      for (size_t i = 0; i < edge.nodes.size() - 1; i++) {
        edge.kmer_indices.insert(edge.kmer_indices.end(), node_kmers_[edge.nodes[i]].begin(), node_kmers_[edge.nodes[i]].end());
      }
    }
  }
}
...
Graph::NodeIdSeq HaplotypeSamplerOverlay::FindBestPath() const {
  ...
  for (const auto & kmer : kmers_) {
    min_score -= kmer.score;
  }

  struct DPState {
    double score;
    odgi::nid_t pred_node;
    int shadow_idx; // If pred_node is a shadow edge, index of the shadow edge in shadow_edges_[pred_node]
  };
  std::vector<DPState> dp(max_id - min_id + 1, DPState{min_score, 0, -1});
  ...
  for (odgi::nid_t i = min_id; i <= max_id; ++i) {
    ...
    // Propagate along real forward edges.
    graph_.follow_edges(graph_.get_handle(i), false /* forward */, [&](const handlegraph::handle_t& next) {
      // >= to ensure we update the predecessor for ties, which can occur when edges don't have k-mers and thus don't change the score
    ...
    // Propagate along shadow edges that start at node i.
    if (auto se_it = shadow_edges_.find(i); se_it != shadow_edges_.end()) {
      const auto& edges = se_it->second;
      for (size_t se_idx = 0; se_idx < edges.size(); ++se_idx) {
        const auto& edge = edges[se_idx];
        double score = state.score;
        for (auto idx : edge.kmer_indices)
          score += 2 * kmers_[idx].score;
        
        if (auto & next_state = dp[edge.nodes.back() - min_id]; score >= next_state.score) {
          next_state.score = score;
          next_state.pred_node = i;
          next_state.shadow_idx = se_idx;
        }
      }
    }
  }
  ...
  while (current != min_id) {
    ...
    if (state.shadow_idx >= 0) {
      // Came via shadow edge with nodes = [h1,...,h_{n}], add h_* in reverse order
      const auto& edge = shadow_edges_.at(state.pred_node).at(state.shadow_idx);
      assert(edge.nodes.back() == current); // h_n is the current node,
      for (auto nit = edge.nodes.rbegin(); nit != edge.nodes.rend(); ++nit) {
        path.push_back(*nit);
      }
    } else {
      // Came via real edge, add current node
...
void HaplotypeSamplerOverlay::UpdateScores(const Graph::NodeIdSeq& path) {
  ...
  for (auto path_it = path.begin(); path_it != path.end(); ++path_it) {
    ...
    // Collect shadow-edge k-mer indices for each node on the path.
    if (auto se_it = shadow_edges_.find(nid); se_it != shadow_edges_.end()) {
      const auto& edges = se_it->second;
      for (const auto& edge : edges) {
        // Does the nodes on this shadow edge match the next nodes on the path?
        auto [edge_it, past_edge_it] = std::mismatch(edge.nodes.begin(), edge.nodes.end(), path_it + 1, path.end());
        if (edge_it == edge.nodes.end()) {
          on_path.insert(edge.kmer_indices.begin(), edge.kmer_indices.end());

          // Update path_it to skip over the shadow edge nodes h1,...,h_{n-1} since we've already processed their k-mers. Step back
          // one because the edge nodes contains the destination that should be processed on the next iteration.
          assert(past_edge_it != path.begin());
          path_it = past_edge_it - 1;
          
          break; // Only one shadow edge can match since they can't share intermediate nodes
        }
      }
    }
  }
  ...
  for (size_t i = 0; i < kmers_.size(); ++i) {
    ...
    switch (km.zygosity) {
      ...
      case KmerZygosity::HOMOZYGOUS:
        ...
        if (here) km.score *= params_.homozygous_discount;
        break;
      case KmerZygosity::HETEROZYGOUS:
        ...
        km.score -= (here ? params_.het_adjustment : -params_.het_adjustment);  
        break;
    }
  }
}
