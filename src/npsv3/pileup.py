...
class Fragment:
    ...
    def fragment_straddles(self, left_region: Range, right_region: Range, min_aligned=3):
        ...
        if self.read1.reference_name != left_region.contig:
            return False
        return (left_region.start <= self.read1.reference_start < (left_region.end - min_aligned)) and (
    ...
    def reads_overlap(self, region: Range, min_overlap=3):
        return (self.read1 and (region.get_overlap(self.read1) >= min_overlap)) or (
            self.read2 and (region.get_overlap(self.read2) >= min_overlap)
        )
        ...
        if isinstance(pileup_item, PileupRead):
            ...
            for col_slice, cigar_op, read_slice in _read_columns(region, pileup_item._read):
                ...
                if align is None:
                    ...
                    align = list(
                        map(_refine_base_alignment, pileup_item._read.query_sequence[read_slice], ref_seq[col_slice])
                    )
